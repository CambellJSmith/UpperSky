...
const UNDERWATER_SHADER: Shader = preload("res://application/effects/underwater_view.gdshader") # Loads the reusable screen-space refraction and blue absorption shader.
...
const FULL_DEPTH_DISTANCE: float = 18.0 # Reaches maximum visual strength after descending this far below the local water level.
...
func _ready() -> void: # Builds the full-screen effect when the node enters the game scene.
    ...
    _material = ShaderMaterial.new() # Allocates one material instance whose depth parameter can change independently.
    _material.shader = UNDERWATER_SHADER # Assigns the authored refraction and colour-absorption shader.
...
func _process(_delta: float) -> void: # Updates underwater visibility and depth strength every rendered frame.
    ...
    var camera_depth: float = maxf(water_level - camera_world_position.y, 0.0) # Measures current depth below the local water surface.
    var depth_factor: float = clampf(camera_depth / FULL_DEPTH_DISTANCE, 0.0, 1.0) # Converts depth into the shader's normalized effect-strength range.
    _material.set_shader_parameter("depth_factor", depth_factor) # Strengthens blue absorption, wobble, and peripheral darkening with depth.
