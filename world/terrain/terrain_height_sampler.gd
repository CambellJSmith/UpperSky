...
const REGIONAL_TIER_NOISE_FREQUENCY: float = 0.000052 # Makes distinct regional elevation provinces recur more often without shrinking them into local hills.
const REGIONAL_TIER_BREAKUP_NOISE_FREQUENCY: float = 0.000115 # Adds more frequent broad contour variation so neighbouring regions do not remain at one elevation for excessive distances.
const MACRO_CONTINENT_WEIGHT: float = 0.50 # Keeps continents dominant while leaving more influence for regional elevation changes.
const MACRO_TIER_WEIGHT: float = 0.34 # Gives secondary elevation provinces enough weight to create more frequent highlands and lowlands.
const MACRO_BREAKUP_WEIGHT: float = 0.16 # Lets broad breakup structure diversify province boundaries without introducing small noisy terrain.
const MACRO_ELEVATION_MINIMUM: float = 0.06 # Allows somewhat deeper low provinces while retaining protection from absolute noise extremes.
const MACRO_ELEVATION_MAXIMUM: float = 0.94 # Allows somewhat higher provinces so upper elevation bands occur more regularly.
const MACRO_SMOOTH_START: float = 0.04 # Begins remapping only near the lowest tail of the combined regional field.
const MACRO_SMOOTH_END: float = 0.96 # Completes remapping only near the highest tail of the combined regional field.
...
const TIER_TRANSITION_START: float = 0.16 # Preserves slightly larger regional interiors while keeping broad traversable climbs between elevation provinces.
const PLATEAU_UNDULATION_HEIGHT: float = 64.0 # Adds stronger rolling uplands and plains so non-mountain regions retain visible character.
const BASIN_DEPTH: float = 150.0 # Creates substantial broad basins and lowlands with gradual margins.
const MOUNTAIN_PROVINCE_NOISE_FREQUENCY: float = 0.000030 # Makes continent-scale tectonic mountain belts recur more often across long-distance travel.
const MOUNTAIN_MASS_NOISE_FREQUENCY: float = 0.000060 # Creates more opportunities for broad supporting mountain mass inside each tectonic belt.
const MOUNTAIN_PROVINCE_START: float = 0.49 # Allows mountain systems across a larger share of coherent tectonic provinces.
const MOUNTAIN_PROVINCE_FULL: float = 0.73 # Reaches full tectonic-province strength in more mountain-belt cores while retaining gradual foothills.
const MOUNTAIN_MASS_START: float = 0.34 # Requires broad supporting crustal mass while allowing more candidate range interiors to develop.
const MOUNTAIN_MASS_FULL: float = 0.66 # Gives full relief to strong broad mountain masses without requiring only the rarest field peaks.
const MOUNTAIN_BASE_HEIGHT: float = 460.0 # Raises complete mountain provinces above their surrounding piedmont with slightly stronger regional presence.
const MOUNTAIN_RIDGE_HEIGHT: float = 1900.0 # Builds exceptional long mountain ranges above broad regional uplands.
const MOUNTAIN_SHOULDER_HEIGHT: float = 560.0 # Adds stronger secondary ridges, foothills, and structural mountain shoulders.
...
const MOUNTAIN_WIDTH_SUPPORT_START: float = 0.36 # Lets more broad ridge bodies contribute substantial relief while still rejecting isolated spikes.
const MOUNTAIN_WIDTH_SUPPORT_END: float = 0.61 # Grants full ridge height once a clearly broad range exists across the support samples.
...
func _init() -> void: # Builds every deterministic field used by the geological terrain hierarchy.
    ...
    _warp_z_noise = _create_noise(23, 0.000075, 3, 0.52, 2.0, FastNoiseLite.FRACTAL_FBM) # Creates independent slow z-axis continental distortion.
    ...
    _tier_noise = _create_noise(41, REGIONAL_TIER_NOISE_FREQUENCY, 4, 0.52, 2.0, FastNoiseLite.FRACTAL_FBM) # Adds more frequently recurring but still broad regional elevation provinces.
    _tier_breakup_noise = _create_noise(53, REGIONAL_TIER_BREAKUP_NOISE_FREQUENCY, 3, 0.48, 2.0, FastNoiseLite.FRACTAL_FBM) # Breaks broad province contours more often without local cliffs.
    ...
    _mountain_province_noise = _create_noise(67, MOUNTAIN_PROVINCE_NOISE_FREQUENCY, 3, 0.54, 2.0, FastNoiseLite.FRACTAL_FBM) # Creates more frequent continent-scale mountain provinces while retaining long coherent belts.
    _mountain_mass_noise = _create_noise(71, MOUNTAIN_MASS_NOISE_FREQUENCY, 4, 0.5, 2.0, FastNoiseLite.FRACTAL_FBM) # Shapes more frequently recurring broad mountain mass inside each province.
...
func sample_height(world_x: float, world_z: float) -> float: # Returns the deterministic terrain height at one world-space horizontal position.
    ...
    var macro_elevation: float = clampf(continent_value * MACRO_CONTINENT_WEIGHT + tier_value * MACRO_TIER_WEIGHT + tier_breakup_value * MACRO_BREAKUP_WEIGHT, 0.0, 1.0) # Combines continental structure with stronger regional variety while retaining normalized bounds.
    macro_elevation = lerpf(MACRO_ELEVATION_MINIMUM, MACRO_ELEVATION_MAXIMUM, smoothstep(MACRO_SMOOTH_START, MACRO_SMOOTH_END, macro_elevation)) # Retains broad geological extremes often enough to expose more elevation provinces.
