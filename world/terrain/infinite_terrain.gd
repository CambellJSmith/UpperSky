extends Node3D # Streams deterministic procedural terrain chunks around a tracked player.
...
var _mesh_builder: TerrainMeshBuilder # Converts height samples into seam-consistent renderable chunk meshes.
...
var _chunks: Dictionary[Vector2i, TerrainChunk] = {} # Stores every currently loaded terrain chunk by world-grid coordinate.
...
func _ready() -> void: # Creates reusable terrain resources before the game composition root initializes the player.
    _height_sampler = TerrainHeightSampler.new() # Creates the deterministic world-height service.
    _terrain_material = _create_terrain_material() # Creates one shared material for every streamed chunk.
    _mesh_builder = TerrainMeshBuilder.new(_height_sampler, _terrain_material) # Creates the isolated mesh-construction service.
...
func _process(_delta: float) -> void: # Advances bounded terrain streaming work each rendered frame.
    ...
    _build_pending_chunks() # Generates a fixed number of missing chunks without blocking the full frame.
...
func _refresh_streaming_set(centre: Vector2i) -> void: # Recalculates all chunks required around a new streaming centre.
    ...
    for chunk_coordinate: Vector2i in chunks_to_remove: # Removes every chunk no longer required around the player.
        ...
        chunk.queue_free() # Releases the chunk node, visual mesh, and any collision at the end of the frame.
...
func _build_chunk_immediately(chunk_coordinate: Vector2i) -> void: # Builds one required chunk outside the ordinary frame budget for safe spawning.
    ...
    _build_chunk(chunk_coordinate) # Generates and installs the requested terrain chunk now.

func _build_chunk(chunk_coordinate: Vector2i) -> void: # Generates one chunk mesh and installs its streamable runtime node.
    var terrain_mesh: ArrayMesh = _mesh_builder.build_chunk_mesh(chunk_coordinate) # Generates vertices, normals, colours, indices, and material assignment.
    ...
    chunk.configure(terrain_mesh) # Assigns the generated geometry and creates its visual and collision nodes.
    _chunks[chunk_coordinate] = chunk # Registers the chunk for streaming, collision, and unloading.
    chunk.set_collision_active(_is_collision_coordinate(chunk_coordinate)) # Enables exact collision immediately when the chunk is near the player.
...
func _rebase_world_if_needed() -> void: # Moves active transforms toward local origin while preserving absolute procedural coordinates.
    ...
    for chunk_coordinate: Vector2i in _chunks.keys(): # Visits every loaded terrain chunk after updating the origin offset.
        var chunk: TerrainChunk = _chunks[chunk_coordinate] # Retrieves the chunk requiring a new near-origin position.
        chunk.position = _get_chunk_local_position(chunk_coordinate) # Repositions the chunk without regenerating its absolute-coordinate geometry.
...
func _update_collision_states() -> void: # Enables exact collision nearby and releases it from distant visual chunks.
