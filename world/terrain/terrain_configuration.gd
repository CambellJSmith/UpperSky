...
const VISUAL_RADIUS: int = 8 # Retains a broad multi-kilometre landscape while reducing loaded terrain chunks from 361 to 289.
const COLLISION_RADIUS: int = 1 # Keeps exact concave terrain collision in nine nearby chunks instead of twenty-five.
...
const CHUNKS_BUILT_PER_FRAME: int = 1 # Builds one expensive terrain and water chunk per frame to reduce streaming spikes.
