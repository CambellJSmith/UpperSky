extends PreciseCliffDungeonSystem # Retains exact A/B transition routing while adding deeply embedded collidable exterior portals and a stable readable interior pocket.
class_name CollidableCliffDungeonSystem # Exposes the complete cliff-only paired-dungeon runtime with precise spawns, exterior collision, and reliable interior movement.
...
const INTERIOR_SAFE_FLOOR_CLEARANCE: float = 0.12 # Gives the capsule a conservative initial gap above generated floor collision before gravity settles it naturally.

func _enter_dungeon(pair_id: int, endpoint: DungeonPairDefinition.Endpoint) -> bool: # Generates the selected readable dungeon in the stable below-world pocket and lands the player at the exact matching interior endpoint.
    ...
    var readable_dungeon: ReadableProceduralDungeonWorld = ReadableProceduralDungeonWorld.new() # Creates the deterministic interior specialization with stronger ambient and lantern illumination.
    _active_dungeon = readable_dungeon # Stores the readable subclass through the existing base dungeon ownership contract.
    ...
    _active_dungeon.build(pair.pair_id, pair.dungeon_seed, pair.region_coordinate) # Reconstructs topology, meshes, collision, doors, fixtures, and lighting from the immutable pair seed.
    ...
    var arrival_position: Vector3 = DungeonSpawnResolver.get_scene_position_in_front(arrival_door, INTERIOR_SAFE_SPAWN_DISTANCE, INTERIOR_SAFE_FLOOR_CLEARANCE) # Places the capsule near the centre of the correct endpoint cell using that door's exact transformed inward-facing axis.
    ...
    _player.global_position = arrival_position # Moves the player to the exact corresponding interior A or B door with conservative collision clearance.
    ...
    return true # Reports successful deterministic entry into the stable readable dungeon pocket.
